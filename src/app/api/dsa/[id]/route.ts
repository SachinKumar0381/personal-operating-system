...
import { updateDsaSchema } from "@/features/dsa-tracker/schemas/dsa-schema";
...
export async function PATCH(
  ...
  try {
    ...
    const parsed = updateDsaSchema.safeParse(body);
    if (!parsed.success) {
      return errorResponse(new Error(parsed.error.issues[0]?.message ?? "Invalid input"), 400);
    }

    const problem = await dsaService.updateProblem(session.user.id, id, parsed.data);
...
export async function DELETE(
  ...
  try {
    ...
    const { id } = await params;
    await dsaService.deleteProblem(session.user.id, id);
