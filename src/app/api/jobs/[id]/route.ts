...
import { updateJobSchema } from "@/features/job-tracker/schemas/job-schema";
...
export async function PATCH(req: NextRequest, { params }: RouteParams): Promise<Response> {
  try {
    ...
    const application = await jobService.updateApplication(session.user.id, id, parsed.data);
...
export async function DELETE(_req: NextRequest, { params }: RouteParams): Promise<Response> {
  try {
    ...
    const { id } = await params;
    await jobService.deleteApplication(session.user.id, id);
